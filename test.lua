-- ==============================================================================
-- BẢN FULL MOD SIÊU TỐI ƯU & FULL CHỨC NĂNG (ĐÃ FIX LỖI SERVER KEY)
-- ==============================================================================
...
_G.ngocdoianConfig = _G.ngocdoianConfig or { 
    CustomMagicBullet = false, AutoHead = false, EspVip = false, EspDistance = false, 
    EspVipPro = false, EspRadar = false, EspLoai5 = false, EspLoai6 = false, 
    Esp7_SoLuong = true, Esp7_VuKhi = true, Esp7_TuThe = true, EspLoai8 = false,
    EspBomMaster = false, EspItemBom = false, EspActiveBom = false, EspAimWarning = false,         
    EspAimWarningVisCheck = false, EspVehicle = false, EspVeh_Dacia = true,  
    EspVeh_UAZ = true, EspVeh_Buggy = true, EspVeh_Coupe = true, EspVeh_Mirado = true, 
    EspVeh_Motor = true, EspVeh_Other = true, Esp3ShowName = true, Esp3ShowHP = true,
    OutlineThickness = 10, UnlockFPS = false, IpadView = false, CustomHRecoil = false,  
    CustomVRecoil = false, LessShake = false, RemoveGrass = false, RemoveTrees = false,  
    RemoveFog = false, WhiteBody = false, ColorBodyV3 = false, ColorBodyNew = false,   
    WallVehicle = false, Crosshair = false, Accuracy = false, BlackSky = false, 
    AimTouchEnable = false, AimTouchHipIgKnock = false, AimTouchHipIgBot = false,
    AimTouchSGIgKnock = false, AimTouchSGIgBot = false, AimTouchHipVisCheck = false,
    AimTouchSGVisCheck = false, AimTouchHipfire = false, AimTouchSG = false,
    AimTouchSGAutoFire = false, AimTouchScopeAll = false, AimTouchScopeIgKnock = false,
    AimTouchScopeIgBot = false, AimTouchScopeVisCheck = false, AimTouchScopeSniper = false,
    AimTouchSniperIgKnock = false, AimTouchSniperIgBot = false, AimTouchSniperVisCheck = false,
    AimTouchCrossbow = false, WeaponGlow = false, BugManEnable = false
}

_G.ngocdoianState = _G.ngocdoianState or { 
    LoopToken = 0, NativeESPReady = false, GraphicsUnlocked = false, MenuStep = 0, 
    LastCmdTime = 0, TrackedMarks = {}, EnemyMarks = {}, LastAimbotCheckTime = 0, 
    CustomTextData = nil, LastAimbotConfigString = "", MagicUpdateVersion = 1,
    LastMagicConfigHash = "", PrevGraphicsState = {}
}
...
function _G.InitializeAutoHeadHooks()
    pcall(function()
        ...
        local modulesToHook = { "GameLua.Mod.BaseMod.Common.Weapon.ShootWeaponEntity", "GameLua.Logic.Weapon.ShootWeaponEntity" }
...
_G.SaveModSettings = function()
    pcall(function()
        local data = "return {\nngocdoianConfig = {\n"
        for k, v in pairs(_G.ngocdoianConfig or {}) do data = data .. "  [\"" .. tostring(k) .. "\"] = " .. tostring(v) .. ",\n" end
        data = data .. "},\nCustomTextData = {\n"
        if _G.ngocdoianState and _G.ngocdoianState.CustomTextData then
            for k, v in pairs(_G.ngocdoianState.CustomTextData) do data = data .. "  [\"" .. tostring(k) .. "\"] = " .. tostring(v) .. ",\n" end
        end
        data = data .. "}\n}"
            ...
            if func then
                ...
                if savedData and type(savedData) == "table" then
                    if savedData.ngocdoianConfig then
                        for k, v in pairs(savedData.ngocdoianConfig) do _G.ngocdoianConfig[k] = v end
                    end
                    if savedData.CustomTextData then
                        _G.ngocdoianState.CustomTextData = _G.ngocdoianState.CustomTextData or {}
                        for k, v in pairs(savedData.CustomTextData) do _G.ngocdoianState.CustomTextData[k] = v end
                    end
                end
            end
        end
...
function _G.InitModMenuTab()
    ...
    if not SettingPageDefine.ModMenu then
        ...
        local StackCombat = {
            { Key = "ModMenu_ColorV3", UI = AliasMap.Switcher, Text = "CHAMS MÀU V3", GetFunc = function() return _G.ngocdoianConfig.ColorBodyV3 end, SetFunc = function(c,v) _G.ngocdoianConfig.ColorBodyV3 = v return true end },
            { Key = "ModMenu_ColorNew", UI = AliasMap.Switcher, Text = "CHAMS MÀU NEW", GetFunc = function() return _G.ngocdoianConfig.ColorBodyNew end, SetFunc = function(c,v) _G.ngocdoianConfig.ColorBodyNew = v return true end },
            ...
            { Key = "ModMenu_BugMan_Ex", UI = AliasMap.TitleSwitcher, Text = "▶ Kéo Dãn Màn Hình", ExpandIndex = 0, GetFunc = function() return _G.ngocdoianConfig.BugManEnable end, SetFunc = function(c,v) _G.ngocdoianConfig.BugManEnable = v return true end },
...
-- ========================================== 
-- LOCAL FUNCTIONS CHO LOGIC NEW WALL VÀ MESH
-- ========================================== 
